...
import { usedCarsData } from '@/components/usedCarsData';
...
const UsedCars = () => {
    ...
    const slicedUsedCarsData = usedCarsData.slice(0, 6);
                ...
                >
                    ...
                    <div className="absolute top-0 inset-0 text-white">
                      ...
                      >
                        <div className="flex items-center gap-0.5">
                          ...
                          <p className="text-[13px] cursor-text">{item.ucNumberOfImages}</p>
                        </div>
                  ...
                  <div className="mx-6 flex-grow">
                    <div className="text-secondary font-semibold mb-3">
                      <h3 className="text-[15px] mb-1">{`$${item.ucAmount}`}</h3>
                      <Link href={`/listings/car-details/${item.UC_}`}><p className="text-[13px]">{item.ucTitle}</p></Link>
                    </div>
                    <div className="flex items-center gap-1 text-[13px] ">
                      {item.ucReviews.map((review, index) => {
                        return (
                      ...
                      <p className="text-[#5F6973]">{`(${item.ucNumberOfReviews} ${
                        item.ucNumberOfReviews === 1 || item.ucNumberOfReviews === 0
                          ? "Review"
                          : "Reviews"
                      })`}</p>
                    </div>
